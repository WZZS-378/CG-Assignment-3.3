...
function buildAsteroidBelt() {
    ...
    for (var i = 0; i < asteroidCount; i++) {
        ...
        var sx = asteroidSize * (0.5 + Math.random()); // random scale between 0.5 and 1.5
...
function buildGui() {
    ...
    gui.add(parameters, 'beltRadius', earthOrbit + 2, earthOrbit + 50).name('Belt Radius').onChange(function(value) {
        beltRadius = value;
        buildAsteroidBelt();
    });
    gui.add(parameters, 'beltWidth', 1, 20).name('Belt Width').onChange(function(value) {
        beltWidth = value;
        buildAsteroidBelt();
    });
    gui.add(parameters, 'asteroidCount', 50, 500).step(1).name('Asteroid Count').onChange(function(value) {
        asteroidCount = value;
        buildAsteroidBelt();
    });
    gui.add(parameters, 'asteroidSize', 0.1, 1).name('Asteroid Size').onChange(function(value) {
        asteroidSize = value;
        buildAsteroidBelt();
    });
    gui.add(parameters, 'rotationSpeed', 0.001, 0.01).name('Rotation Speed').onChange(function(value) {
        rotationSpeed = value;
        buildAsteroidBelt();
    });
